...
default_prompt: "用 futa-balance 设计/校准符塔数值：先改 scripts/balance-sim.mjs 旋钮跑到五条全绿，再落配置层。"
description: |
  《符塔》(玄幻无限爬塔塔防 roguelite)的数值设计与平衡验证方法论。当需要设计、调整、
  审查任何游戏数值时使用：伤害/承伤/战力、经验与境界、怪物成长、深度硬墙、疲劳度、
  Boss 轮换、地图随机属性、副本倍率、经济产出。
  铁律：任何数值改动先过 `scripts/balance-sim.mjs` 的崩盘检测(五条曲线全绿)，再落进 src/config。
...
> **改旋钮 → 跑模拟器 → 五条全绿 → 落配置 → 四层门禁 → 验收。** 绝不"拍脑袋给数字"，也绝不"他说合理就过"。

```text
1. 改 scripts/balance-sim.mjs 顶部 KNOBS（一次只动 1-2 个，便于归因）
2. node scripts/balance-sim.mjs  (或 npm run balance)
3. 读"崩盘检测总判"：
   ① 深度墙平滑  ② 承伤减伤恒在 25~75%  ③ 疲劳会话长度
   ④ Boss 复现频率受控  ⑤ exp/破墙经济收敛  ⑥ 深度采样表
   - 有 ❌ → 调旋钮重跑，直到五条全绿
4. 把验证过的数值写进 src/config/**（纯数据 + schema 校验）
...
- **尺子归 Claude**：`balance-sim.mjs`、`check-config.mjs`、`golden.formulas.test.ts`、`config.smoke.gate.test.ts`、`contracts.json` —— Codex 只读。

## 符塔当前锁定值（v4，详见 docs/source/符塔_无限模式_v4.md）

深度成长 g=1.07；承伤相对式恒 45%；疲劳 −1.2%战力/层(舒适~34层/会话)；Boss 重铸精英 0.75R、每3关第5波；exp(d)=100×1.07^(d-1)；破墙 +6%战力/会话(+10层≈12会话)；副本 ×1.2~1.5 无疲劳。改这些 → 先过 balance-sim。
