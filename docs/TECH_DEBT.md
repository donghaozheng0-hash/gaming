...
| 3 | 浏览器 dev 实时调参台(拖 KNOBS 重算曲线红绿 + override 同步到运行中战斗)尚未落地,数值核已就绪但只有 CLI 消费方 | T6 拆批 (2026-07-03) | T6.1 紧随批次:dev-only(`?debug=1`)面板 import `balance-core.mjs`;运行时 override 接口 T6 已预留(注入式 config);调参产物仍须过 balance-sim 全曲线绿才落配置(尺子判定权不变) | 还债中(T6.1,契约+尺子已钉 2026-07-05) |
