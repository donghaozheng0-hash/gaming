...
//   · Codex【不得修改本文件的期望值】(663/995/1326/444/80)。若认为算例有误，必须回报 Claude 评审，不得自行改测试就标"通过"。这是反套娃的关键护栏。
//
// 接口契约(如需变更必须经 Claude 评审，不允许 Codex 单方面改签名)：
//   damage.runeDamage({ base, qualityMul, xiangshengMul, kezhiMul, drawBonus? }): number   // 四舍五入到整数
//   core.coreDamage({ atk, def, k }): number                                                // 减伤 = def/(def+k)
//
...
describe("总设计算例 · 阵眼承伤 (减伤常数 K=600)", () => {
  it.skipIf(!existsSync(corePath))(
    "DEF150 → 减伤20% → 怪攻100 → 阵眼掉 80",
    async () => {
      const { coreDamage } = await load(corePath);
      expect(coreDamage({ atk: 100, def: 150, k: 600 })).toBe(80);
    },
  );
});
